...
public class FishingTaskManager {
    ...
    public static List<FishingTaskStruct> getRoomTaskList(ServerUser user) {
        ...
        if (taskStructList == null) { // 内存没有任务数据，重新生成
            ...
            if (taskMap.size() > 0) {
                // 直接读取已经有的任务 房间任务只有一个
                FishingTaskStruct taskStruct =
                    GsonManager.gson.fromJson(taskMap.values().iterator().next(), FishingTaskStruct.class);
    ...
    private static void getNewTask(ServerUser user, TaskType taskType, List<FishingTaskStruct> taskList) {
        List<TaskConfig> taskConfigs = DataContainer.getDatas(TaskConfig.class).stream()
            .filter(taskConfig -> taskConfig.getTaskType() == taskType.getId()).collect(Collectors.toList());
        // String pattern;
        ...
        if (taskType == TaskType.ROOM) {
            // 领取新任务时之前的要删除掉
            String configId = RedisHelper.get("ROOM_LAST_CONFIG_ID" + user.getId());
            int a = 0;
            if (!configId.isEmpty()) {
                a = Integer.parseInt(configId);
                if (a >= taskConfigs.size()) {
                    a = 0;
                } else {
                    a++;
                }
            ...
            for (int i = 0; i < taskConfigs.size(); i++) {
                if (a == i) {
                    ...
                    taskStruct.setId(taskConfigs.get(i).getId());
                    taskStruct.setProgress(0);
                    taskStruct.setTarget(taskConfigs.get(i).getGoalNum());
                    taskStruct.setActiveLevel(taskConfigs.get(i).getActiveLevel());
                    taskStruct.setReceived(false);
                    taskStruct.setCreateTime(new Date());

                    List<ItemData> realRewards = taskConfigs.get(i).getRealRewards();
                    ...
                    RedisHelper.set("ROOM_LAST_CONFIG_ID" + user.getId(), i + "");
                    // 放入任务列表
                    taskList.add(taskStruct);

                    String key = String.format(taskType == TaskType.ROOM ? TASK_ROOM_NAMESPACE : TASK_DAILY_NAMESPACE,
                        Long.toString(user.getId()), Long.toString(taskConfigs.get(i).getId()));
            ...
            for (TaskConfig taskConfig : taskConfigs) {
                ...
                taskStruct.setProgress(0);
                taskStruct.setTarget(taskConfigs.get(1).getGoalNum());
                ...
                String key = String.format(taskType == TaskType.ROOM ? TASK_ROOM_NAMESPACE : TASK_DAILY_NAMESPACE,
                    Long.toString(user.getId()), Long.toString(taskConfig.getId()));
                String value = GsonManager.gson.toJson(taskStruct);
                // 存入Redis
                RedisHelper.set(key, value);
    ...
    public static void doTask(ServerUser user, TaskType taskType, GoalType goalType, int goalId, int size) {
        ...
        for (FishingTaskStruct taskStruct : taskList) {
            ...
            if (taskStruct.isReceived() || taskStruct.getProgress() >= config.getGoalNum()) { // 已领取或已完成就不管
                continue;
            }
            // 玩家做任务
            if (config.getGoalType() == goalType.getId() && config.getRealGoalId().contains(goalId)) {
                int target = config.getGoalNum();
                ...
                String key = String.format(taskType == TaskType.ROOM ? TASK_ROOM_NAMESPACE : TASK_DAILY_NAMESPACE,
                    Long.toString(user.getId()), Long.toString(taskStruct.getId()));
                // 更新redis任务数据
     ...
     * 做任务
     */
    public static void doTask1(ServerUser user) {
        if (StringUtils.isEmpty(user.getConnect())) { // 机器人不做任务
            return;
        }

        List<FishingTaskStruct> taskList;
        TaskType taskType = TaskType.ROOM;
        taskList = getRoomTaskList(user);
        for (FishingTaskStruct taskStruct : taskList) {
            TaskConfig config = DataContainer.getData(taskStruct.getId(), TaskConfig.class);
            if (taskStruct.isReceived()) { // 已领取或已完成就不管
                continue;
            }
            // 玩家做任务
            taskStruct.setProgress(0);
    ...
    public static void getTaskReward(ServerUser user, TaskType taskType, long taskId) {
        ...
        if (taskType == TaskType.DAILY) {
            ...
            taskList = getDailyTaskList(user).stream().filter(taskStruct -> taskStruct.getId() == taskId).findFirst();
        } else {
            key = String.format(TASK_ROOM_NAMESPACE, Long.toString(user.getId()), Long.toString(taskId));
            taskList = getRoomTaskList(user).stream().filter(taskStruct -> taskStruct.getId() == taskId).findFirst();
        }
        ...
        TaskConfig config = DataContainer.getData(task.getId(), TaskConfig.class);
        if (task.getProgress() < config.getGoalNum()) {
        ...
        for (ItemData reward : task.getTaskRewards()) {
            ...
            itemDataProtoList.add(ItemDataProto.newBuilder().setItemId(rewardItemId).setItemNum(rewardCount).build());
        ...
        if (taskType == TaskType.DAILY) {
            OseeLobbyMessage.GetDailyTaskRewardResponse.Builder builder =
                OseeLobbyMessage.GetDailyTaskRewardResponse.newBuilder();
            ...
            OseeFishingMessage.FishingGetRoomTaskRewardResponse.Builder builder =
                OseeFishingMessage.FishingGetRoomTaskRewardResponse.newBuilder();
            builder.addAllRewards(itemDataProtoList);
    ...
    public static void getActiveReward(ServerUser user, int activeLevel) {
        ...
        String key = String.format(TASK_DAILY_ACTIVEREWARD_NAMESPACE, Long.toString(user.getId()),
            Integer.toString(activeLevel));
        ...
        OseeLobbyMessage.GetDailyActiveRewardResponse.Builder builder =
            OseeLobbyMessage.GetDailyActiveRewardResponse.newBuilder();
        builder.addRewards(ItemDataProto.newBuilder().setItemId(itemId).setItemNum(itemNum).build());
    ...
    public static OseePublicData.TaskInfoProto createTaskInfoProto(FishingTaskStruct taskStruct) {
        ...
        builder.setTarget(taskConfig.getGoalNum());
        builder.setReceive(taskStruct.isReceived());
        builder.setGoalId(taskConfig.getFishId());
        builder.setGoalType(taskConfig.getGoalType());
        ItemData[] rewardArray = new Gson().fromJson(taskConfig.getRewards(), ItemData[].class);
        for (ItemData taskReward : rewardArray) {
            builder.addRewards(
                ItemDataProto.newBuilder().setItemId(taskReward.getItemId()).setItemNum(taskReward.getCount()).build());
        }
    ...
    public static void sendDailyTaskListResponse(ServerUser user) {
        ...
        for (Integer active : activeLevel) {
            String key =
                String.format(TASK_DAILY_ACTIVEREWARD_NAMESPACE, Long.toString(user.getId()), Integer.toString(active));
            String value = RedisHelper.get(key);
            ...
            builder.addActive(
                OseeLobbyMessage.ActiveLevelInfoProto.newBuilder().setActiveLevel(active).setReceive(receive).build());
        }
        // 任务详情
        for (FishingTaskStruct taskStruct : dailyTaskList) {
    ...
    public static void sendRoomTaskListResponse(ServerUser user) {
        ...
        OseeFishingMessage.FishingRoomTaskListResponse.Builder builder =
            OseeFishingMessage.FishingRoomTaskListResponse.newBuilder();
        // 房间任务详情
        for (FishingTaskStruct taskStruct : taskList) {
    ...
    public static void oneKeyGetDailyTaskRewards(ServerUser user) {
        ...
        for (FishingTaskStruct task : taskList) {
            TaskConfig config = DataContainer.getData(task.getId(), TaskConfig.class);
            if (task.getProgress() == config.getGoalNum() && !task.isReceived()) { // 已完成但还没有领取
                // 领取奖励
                String key =
                    String.format(TASK_DAILY_NAMESPACE, Long.toString(user.getId()), Long.toString(task.getId()));
        ...
        OseeLobbyMessage.OneKeyGetDailyTaskRewardsResponse.Builder builder =
            OseeLobbyMessage.OneKeyGetDailyTaskRewardsResponse.newBuilder();
        for (Map.Entry entry : rewards.entrySet()) {
            builder.addRewards(ItemDataProto.newBuilder().setItemId((Integer)entry.getKey())
                .setItemNum((Long)entry.getValue()).build());
        }
