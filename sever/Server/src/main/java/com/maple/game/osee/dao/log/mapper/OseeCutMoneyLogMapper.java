...
public interface OseeCutMoneyLogMapper {
     ...
     * 获取今日指定游戏的所有抽水
     */
    @Select("SELECT COALESCE(SUM(`cut_money`), 0) totalCut FROM " + TABLE_NAME
        + " WHERE DATE(`create_time`) = CURDATE()" + " AND `game` = #{game}")
    ...
    @Update("CREATE TABLE IF NOT EXISTS " + TABLE_NAME + " ("
        ...
        + "`user_id` bigint(20) NULL COMMENT '玩家id'," + "`game` int(11) NOT NULL COMMENT '游戏',"
        + "`type` int(11) NOT NULL COMMENT '游戏'," + "`cut_money` bigint(20) NULL COMMENT '变动金币',"
