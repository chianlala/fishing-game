...
public interface StockMapper {
    ...
    @Update("CREATE TABLE IF NOT EXISTS " + TABLE_NAME + " ("
        ...
        + "  `shop_id` bigint(20) NOT NULL COMMENT '道具库存属于的商品ID'," + "  `user_id` bigint(20) NULL COMMENT '兑换该卡的玩家ID',"
        + "  `number` varchar(100) NOT NULL COMMENT '卡号'," + "  `password` varchar(100) NOT NULL COMMENT '卡密',"
     ...
     * 保存数据
     */
    @Insert("insert into " + TABLE_NAME + "(`shop_id`,`user_id`,`number`,`password`)" + " values "
     ...
     * 更新数据
     */
    @Update("update " + TABLE_NAME + " set " + "`shop_id` = #{entity.shopId}, `user_id` = #{entity.userId},"
        + "`number` = #{entity.number}, `password` = #{entity.password}" + " where `id` = #{entity.id}")
