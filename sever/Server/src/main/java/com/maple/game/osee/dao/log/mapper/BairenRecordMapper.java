...
import com.maple.game.osee.dao.log.entity.BaiRenRecordLogEntity;

/**
 * 飞禽走兽游戏记录表
 */
@Mapper
public interface BairenRecordMapper {
    String TABLE_NAME = "tbl_osee_bairen_record_log";
    ...
    void save(@Param("entity") BaiRenRecordLogEntity entity);
    ...
    int saveList(@Param(value = "testLists") List<BaiRenRecordLogEntity> testLists);

    /**
     * 根据条件查询记录
     */
    @Select("SELECT * FROM " + TABLE_NAME + " log ${where} ORDER BY log.id DESC ${page}")
    List<BaiRenRecordLogEntity> getLogList(@Param("where") String where, @Param("page") String page);
