...
public class FishConfig extends BaseCsvData {
    ...
    /**
     * 号角召唤鱼：-1 则不能召唤，否则则表示：要消耗的号角数量 0 表示免费
     */
    private int HornCallsFish = -1;
     ...
     * 使用boss号角的间隔时间，单位：秒
     */
    private int bossBugleDelayTime = 60;
    ...
    /**
     * 怪物攻击一次需要的时间集合字符串，例如：2000，单位：毫秒
     */
    private String monsterAttackTime;

    @Nullable
    public List<Integer> getMonsterAttackTimeList() {

        try {

            List<Integer> resList = JSONUtil.toList(monsterAttackTime, Integer.class);

            return resList;

        } catch (Exception e) {

            Integer monsterAttackTimeInt = Convert.toInt(monsterAttackTime);

            return monsterAttackTimeInt == null ? new ArrayList<>() : CollUtil.newArrayList(monsterAttackTimeInt);
    ...
    /**
     * 机器人使用boss号角，召唤boss的权重
     */
    private int robotBossBugleWeight;
