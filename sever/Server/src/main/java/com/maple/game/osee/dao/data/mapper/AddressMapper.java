...
public interface AddressMapper {
    ...
    @Update("CREATE TABLE IF NOT EXISTS " + TABLE_NAME + " ("
        ...
        + "  `player_id` bigint(20) NOT NULL COMMENT '玩家ID'," + "  `name` varchar(50) NOT NULL COMMENT '玩家昵称',"
        + "  `phone` varchar(11) NOT NULL COMMENT '玩家手机号码'," + "  `address` varchar(500) NOT NULL COMMENT '玩家收货地址',"
     ...
     * 保存数据
     */
    @Insert("insert into " + TABLE_NAME + " (player_id, name, phone, address)" + " values "
     ...
     * 更新数据
     */
    @Update("update " + TABLE_NAME + " set " + "player_id = #{entity.playerId}, name = #{entity.name}, "
        + "phone = #{entity.phone}, address = #{entity.address} " + "where id = #{entity.id}")
