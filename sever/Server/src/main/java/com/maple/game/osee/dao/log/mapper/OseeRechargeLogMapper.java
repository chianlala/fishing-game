...
import com.maple.game.osee.dao.data.entity.FeedBackEntity;
...
public interface OseeRechargeLogMapper {
     ...
     * 获取玩家今日充值总额
     */
    @Select("select COALESCE(sum(pay_money), 0) total from " + TABLE_NAME
        + " where recharge_type != 3 and recharge_type != 4 and order_state = 1"
        + " and DATE(`create_time`) = CURRENT_DATE()" + " and user_id = #{userId}")
     ...
     * 根据条件查询充值记录
     */
    @Select("SELECT * FROM app_data.tbl_change_all_log log ${where} ORDER BY `id` DESC ${page}")
    List<ChangeAll> getChangeAllList(@Param("where") String where, @Param("page") String page);

    /**
     * 根据条件查询充值统计值
     */
    @Select("SELECT COUNT(*) totalNum FROM app_data.tbl_change_all_log log ${where}")
    long getChangeAllCount(@Param("where") String where);

    /**
     * 根据条件查询充值记录
     */
    @Select("SELECT * FROM app_data.tbl_agent_player_all log ${where} ORDER BY `id` DESC ${page}")
    List<AgentPlayerAll> getAgentPlayerAllList(@Param("where") String where, @Param("page") String page);

    /**
     * 根据条件查询充值统计值
     */
    @Select("SELECT COUNT(*) totalNum FROM app_data.tbl_agent_player_all log ${where}")
    long getAgentPlayerAllCount(@Param("where") String where);

    /**
     * 保存数据
     */
    @Insert("INSERT INTO app_data.tbl_osee_feed_back (`userId`, `context`, `userName`, `createTime`) VALUES ( "
        + "#{entity.userId}, #{entity.context}, #{entity.userName}, #{entity.createTime})")
    void saveFeedBack(@Param("entity") FeedBackEntity entity);

    /**
     * 根据条件查询充值记录
     */
    @Select("SELECT * FROM app_data.tbl_osee_feed_back log ${where} ORDER BY `createTime` DESC ${page}")
    List<FeedBackEntity> getFeedBackList(@Param("where") String where, @Param("page") String page);

    /**
     * 根据条件查询充值统计值
     */
    @Select("SELECT COUNT(*) totalNum FROM app_data.tbl_osee_feed_back log ${where}")
    long getFeedBackCount(@Param("where") String where);

    @Delete({"DELETE FROM app_data.tbl_osee_feed_back WHERE id=#{id}"})
