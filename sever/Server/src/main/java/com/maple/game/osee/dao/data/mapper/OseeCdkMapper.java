...
public interface OseeCdkMapper {
    ...
    @Insert("INSERT INTO " + TABLE_NAME
        + " (`cdkey`, `type_id`, `rewards`, `user_id`, `user_game_id`, `nickname`, `agent_id`, `agent_game_id`) "
        + "VALUES (#{entity.cdk}, #{entity.typeId}, #{entity.rewards}, #{entity.userId}, #{entity.userGameId}, #{entity.nickname} , #{entity.agentId}, #{entity.agentGameId})")
    ...
    @Update("UPDATE " + TABLE_NAME
        + " SET `user_id`=#{entity.userId}, `user_game_id`=#{entity.userGameId}, `nickname`=#{entity.nickname} "
        + "WHERE `id`=#{entity.id}")
    ...
    @Update("CREATE TABLE IF NOT EXISTS " + TABLE_NAME + " (`id` bigint(20) NOT NULL AUTO_INCREMENT COMMENT 'cdk id', "
        ...
        + "`cdkey` varchar(32) NULL DEFAULT NULL COMMENT 'cdkey内容', " + "`type_id` bigint(20) NOT NULL COMMENT '类型id', "
        ...
        + "`agent_id` bigint DEFAULT NULL COMMENT '所属代理的用户 id，没有所属代理时，则为 0', "
        + "PRIMARY KEY (`id`) USING BTREE) ENGINE = InnoDB AUTO_INCREMENT = 100001")
    void createTable();

    @Select("SELECT b.id, b.`name` FROM tbl_osee_cdk a LEFT JOIN tbl_osee_cdk_type b ON b.id = a.type_id WHERE a.agent_id = #{agentId}")
    Set<GmCdkTypeInfo> getAllTypeIdByAgentId(@Param("agentId") Long agentId);
