...
public interface AppGameLogMapper {
    ...
    @Insert("INSERT INTO " + TABLE_NAME + "("
        + "`number`, `income`, `reward_id`, `stock`, `mode`, `create_time`, `type`" + ") VALUES ("
    ...
    @Select("<script>" + "SELECT\n" + "tbl1.id,\n" + "tbl1.number,\n" + "tbl1.income,\n" + "tbl1.stock,\n"
        ...
        + "tbl_app_game_log AS tbl1 ,\n" + "tbl_app_reward_log AS tbl2\n" + "<where>" + "tbl1.reward_id = tbl2.id "
        + "<if test='startDate != null'>" + "and Date(tbl1.`create_time`) BETWEEN #{startDate} AND #{endDate}" + "</if>"
        + "<if test= 'mode != null'>" + "and #{mode} = tbl1.`mode`" + "</if>" + "<if test= 'type != null'>"
        + "and #{type} = tbl1.`type`" + "</if>" + "</where>" + "order by tbl1.create_time desc LIMIT #{start}, #{end} "
        + "</script>")
    List<AppGameLogEntity> find(@Param("startDate") String startDate, @Param("endDate") String endDate,
        @Param("mode") Integer mode, @Param("start") int start, @Param("end") int end, @Param("type") Integer type);

    @Select("<script>" + "SELECT\n" + "count(tbl1.id) \n" + "FROM\n" + "tbl_app_game_log AS tbl1 ,\n"
        + "tbl_app_reward_log AS tbl2\n" + "<where>" + "tbl1.reward_id = tbl2.id " + "<if test='startDate != null'>"
        + "and Date(tbl1.`create_time`) BETWEEN #{startDate} AND #{endDate}" + "</if>" + "<if test= 'mode != null'>"
        + "and #{mode} = tbl1.`mode`" + "</if>" + "<if test= 'type != null'>" + "and #{type} = tbl1.`type`" + "</if>"
        + "</where>" + "</script>")
    int count(@Param("startDate") String startDate, @Param("endDate") String endDate, @Param("mode") Integer mode,
