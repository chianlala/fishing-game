...
public interface AgentMapper {
     ...
     * 获取代理下级在线人数
     */
    @Select("select coalesce(count(`agent`.id), 0) from " + TABLE_NAME + " `agent`, " + UserMapper.TABLE_NAME
        + " `user`" + " where `agent`.player_id = `user`.id and `agent`.agent_player_id = #{agentPlayerId}"
     ...
     * 获取代理下今日活跃的玩家
     */
    @Select("    select coalesce(count(*), 0) " + "    from app_data.tbl_ttmy_agent a "
        + "    join app_log.tbl_app_login_log l on a.player_id = l.user_id and date(l.exit_time) = curdate() "
        + "    where agent_player_id = #{agentPlayerId}")
    int getDailyActivePlayerSize(@Param("agentPlayerId") Long agentPlayerId);
     ...
     * 获取渠道商今日新增玩家数量
     */
    @Select("select coalesce(count(player.id), 0) from " + TABLE_NAME + " player "
        + " where player.agent_player_id = #{channelId} and " + "date(player.create_time) = curdate()")
    int getDailyAgentPlayerSize(@Param("channelId") Long channelId);
     ...
     * 根据年份获取新增玩家数量
     */
    @Select("select MONTH(create_time) month, COUNT(*) count from " + TABLE_NAME
        + " where agent_player_id = #{agentPlayerId}" + " and YEAR(create_time) = #{year} GROUP BY MONTH(create_time)")
    List<Map<Object, Object>> getPerMonthPlayerSize(@Param("agentPlayerId") Long agentPlayerId,
    ...
    List<AgentEntity> getByAgentPlayerIdWithPage(@Param("agentPlayerId") Long agentPlayerId,
    ...
    @Select("select * from " + TABLE_NAME + " player, " + TABLE_NAME + " promoter, " + TABLE_NAME + " channel, "
        ...
        + "where player.agent_player_id = promoter.player_id " + "AND promoter.agent_player_id = channel.player_id "
        + "AND player.player_id = userInfo.id " + "AND player.player_id = playerInfo.user_id  "
        + "AND channel.player_id = #{channelId} ${condition} order by player.id DESC ${page}")
    List<Map<String, Object>> getPlayerByChannelId(@Param("channelId") Long channelId,
        @Param("condition") String condition, @Param("page") String page);

    /**
     * 获取指定渠道下的玩家数量
     */
    @Select("select count(player.id) from " + TABLE_NAME + " player, " + TABLE_NAME + " promoter, " + TABLE_NAME
        + " channel, " + UserMapper.TABLE_NAME + " userInfo, " + OseePlayerMapper.TABLE_NAME + " playerInfo "
        + "where player.agent_player_id = promoter.player_id " + "AND promoter.agent_player_id = channel.player_id "
        + "AND player.player_id = userInfo.id " + "AND player.player_id = playerInfo.user_id  "
     ...
     * 获取指定渠道下的所有玩家
     */
    @Select("select * from " + TABLE_NAME + " player, " + UserMapper.TABLE_NAME + " userInfo, "
        + OseePlayerMapper.TABLE_NAME + " playerInfo " + "where player.player_id = userInfo.id "
        + "AND player.player_id = playerInfo.user_id  "
        + "AND player.agent_player_id = #{channelId} ${condition} order by player.id DESC ${page}")
    List<Map<String, Object>> getPlayerByAgentId(@Param("channelId") Long channelId,
        @Param("condition") String condition, @Param("page") String page);

    /**
     * 获取指定渠道下的玩家数量
     */
    @Select("select count(player.id) from " + TABLE_NAME + " player, " + UserMapper.TABLE_NAME + " userInfo, "
        + OseePlayerMapper.TABLE_NAME + " playerInfo " + "where player.player_id = userInfo.id "
        + "AND player.player_id = playerInfo.user_id  " + "AND player.agent_player_id = #{channelId} ${condition}")
    int getPlayerByAgentIdSize(@Param("channelId") Long channelId, @Param("condition") String condition);
    ...
    @Insert("insert into " + TABLE_NAME + " "
        ...
        + " invite_qrcode_img, invite_url, state, bank, real_name, bank_num, open_bank,first_pay_rate, other_pay_rate, pay_way)"
        + " values "
        ...
        + " #{agent.inviteQrCodeImg}, #{agent.inviteUrl}, #{agent.state}, #{agent.bank}, #{agent.realName}, #{agent.bankNum}, #{agent.openBank},#{agent.firstPayRate},"
        + "#{agent.otherPayRate}, #{agent.payWay} )")
     ...
     * 更新数据
     */
    @Update("update " + TABLE_NAME + " set " + "player_id = #{agent.playerId}, player_name = #{agent.playerName}, "
        ...
        + "bank = #{agent.bank}, real_name = #{agent.realName}, bank_num = #{agent.bankNum}, open_bank = #{agent.openBank},first_pay_rate = #{agent.firstPayRate},"
        + "other_pay_rate = #{agent.otherPayRate}, pay_way = #{agent.payWay} " + "where id = #{agent.id}")
    int update(@Param("agent") AgentEntity agentEntity);

    /**
     * 更新某代理玩家下面所有玩家的代理等级
     */
    @Update("update " + TABLE_NAME + " set " + "agent_level=#{level} " + "where agent_player_id=#{agentId}")
    ...
    void updateSameSecondRate(@Param("oldRate") Double oldRate, @Param("newRate") Double newRate,
     ...
     * 获取所有渠道
     */
    @Select("select * from " + TABLE_NAME + " where agent_level = 1")
    List<AgentEntity> getAllAgent();

    /**
     * 获取所有渠道
     */
    @Select("select player_id from " + TABLE_NAME + " where agent_level = 1")
    List<Long> getAllAgentPlayerId();

    /**
     * 通过id获取渠道
     */
    @Select("select * from " + TABLE_NAME + " where id = #{id}")
    AgentEntity getAgentById(@Param("id") long id);

    /**
     * 通过用户id获取渠道
     */
    @Select("select * from " + TABLE_NAME + " where player_id = #{playerId}")
    AgentEntity getAgentByPlayerId(@Param("playerId") long playerId);

    /**
     * 修改所有值为新值
     */
    @Update("update " + TABLE_NAME + " set agent_player_id = #{agentPlayerId} where player_id = #{playerId}")
    void updateByPlayerId(@Param("playerId") long playerId, @Param("agentPlayerId") Long agentPlayerId);
