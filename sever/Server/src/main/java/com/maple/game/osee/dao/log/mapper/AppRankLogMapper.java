...
public interface AppRankLogMapper {
    ...
    @Select("SELECT\n" + "tb1.id,\n" + "tb1.player_id,\n" + "tb1.nickname,\n" + "tb1.score,\n" + "tb1.rank,\n"
        ...
        + " AND tb1.`mode` = #{mode}\n ${whereOther}" + " LIMIT #{start}, #{end}")
    List<AppRankLogEntity> find(@Param("start") int start, @Param("end") int end, @Param("date") String date,
        @Param("type") int type, @Param("mode") int mode, @Param("whereOther") String whereOther);

    @Insert({"INSERT INTO " + TABLE_NAME + "("
        + "`player_id`, `nickname`, `score`, `rank`, `mode`, `cost`, `games`, `change`, `reward_id`, `receive`, `receive_time`, `create_time`, `email_id`, `type`"
        + ") VALUES (" + "#{entity.playerId}, #{entity.nickname}, #{entity.score}, #{entity.rank}, #{entity.mode},"
    ...
    @Select("SELECT\n" + "count(tb1.id)" + "FROM\n" + "tbl_app_rank_log AS tb1\n"
        + "LEFT JOIN tbl_app_reward_log AS tb2 ON tb1.reward_id = tb2.id\n" + "WHERE Date(tb1.create_time) = #{date}\n"
        + "AND tb1.`type` = #{type}" + " AND tb1.`mode` = #{mode}\n")
    int count(@Param("date") String date, @Param("type") int type, @Param("mode") Integer mode);
