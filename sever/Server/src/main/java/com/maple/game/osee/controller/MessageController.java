...
public class MessageController {
    ...
    public void receiveMessageItems(OseeLobbyMessage.ReceiveMessageItemsRequest request, ServerUser user) {
        messageManager.receiveMessageItems(request.getId(), user, null);
    }

    /**
     * 删除消息
     */
    @AppHandler(msgCode = OseeMessage.OseeMsgCode.C_S_TTMY_DELETE_MESSAGE_REQUEST_VALUE)
    public void deleteMessage(OseeLobbyMessage.DeleteMessageRequest request, ServerUser user) {
        messageManager.deleteMessage(request.getId(), user);
    }
