...
public interface OseeFruitRecordLogMapper {
    ...
    @Insert("INSERT INTO " + TABLE_NAME
        + " (`money`, `playerId`, `nickname`, `playBeforeMoney`, `playAfterMoney`, `cost`, `lineNum`, `totalWin`, `info`) "
     ...
     */
    @Select("SELECT COALESCE(SUM(money),0) totalMoney, COALESCE(SUM(cost),0) totalCost,COALESCE(SUM(totalWin),0) AllTotalWin  FROM "
        + TABLE_NAME + " log ${where}")
    ...
    @Update("CREATE TABLE IF NOT EXISTS " + TABLE_NAME + " (`id` bigint(20) NOT NULL AUTO_INCREMENT COMMENT '记录id',"
        ...
        + "`money` bigint(20) NULL COMMENT '账户金币变动数额'," + "`playerId` bigint(20) NULL DEFAULT NULL COMMENT '玩家id',"
        ...
        + "`cost` bigint(20) NULL DEFAULT NULL COMMENT '下注消耗'," + "`lineNum` int(20) NULL DEFAULT NULL COMMENT '下注条数',"
        + "`totalWin` bigint(20) NULL DEFAULT NULL COMMENT '中奖赢取',"
        + "`info` varchar(255) NULL DEFAULT NULL COMMENT '中奖详情'," + "`type` int(255) NULL DEFAULT NULL COMMENT '类型',"
     ...
     * 从抽水表中 累加计算 水果拉霸 game = 5 抽水总额 ljy
     */
    @Select("SELECT COALESCE(SUM(cut_money),0) totalCut FROM tbl_osee_cut_money_log WHERE game=${game} ")
    long getTotalCutMoneyByGame(@Param("game") Integer game);

    /**
     * 从抽水表中算出今天龙晶战场的抽水总额
     */
    @Select("SELECT COALESCE(SUM(cut_money),0) totalCut FROM tbl_osee_cut_money_log WHERE game=${game} and type=${type} and date(create_time) = curdate() ")
    long getTotalCutMoneyByDate(@Param("game") Integer game, @Param("type") Integer type);
}
