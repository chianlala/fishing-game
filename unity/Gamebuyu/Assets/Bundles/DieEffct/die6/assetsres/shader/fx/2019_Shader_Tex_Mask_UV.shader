  ...
  SubShader
  {
    ...
    Pass // ind: 1, name: 
    {
      ...
      OUT_Data_Vert vert(appdata_t in_v)
      {
          ...
          u_xlat0.xy = (_Time.xx * float2(_ScrollX, _ScrollY));
          u_xlat0.zw = (_Time.xx * float2(_Scroll2X, _Scroll2Y));
          u_xlat0 = frac(u_xlat0);
          u_xlat1.xy = TRANSFORM_TEX(in_v.texcoord.xy, _MainTex1);
          u_xlat1.zw = TRANSFORM_TEX(in_v.texcoord.xy, _MainTex2);
          u_xlat0 = (u_xlat0 + u_xlat1);
          u_xlat1.xy = u_xlat0.xy;
          u_xlat1.xy = clamp(u_xlat1.xy, 0, 1);
          u_xlat1.xy = ((-u_xlat0.xy) + u_xlat1.xy);
          out_v.texcoord.xy = ((float2(_MainTex1_Wrap, _MainTex1_Wrap) * u_xlat1.xy) + u_xlat0.xy);
      ...
      OUT_Data_Frag frag(v2f in_f)
      {
          ...
          u_xlat1_d.xy = ((u_xlat10_0.xx * _UVXX.xx) + in_f.texcoord.zw);
          u_xlat5.xy = u_xlat1_d.xy;
          u_xlat5.xy = clamp(u_xlat5.xy, 0, 1);
          u_xlat5.xy = ((-u_xlat1_d.xy) + u_xlat5.xy);
          u_xlat1_d.xy = ((float2(float2(_MainTex2_Wrap, _MainTex2_Wrap)) * u_xlat5.xy) + u_xlat1_d.xy);
