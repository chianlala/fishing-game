...
public class UIDoScaleYActive : MonoBehaviour
{
    ...
    void OnEnable()
    {
       // UIDoPool.g_listDoMove.Add(this);
        ...
        if (m_fScaleOut != 1)
        {
            this.transform.localScale = new Vector3(this.transform.localScale.x, m_fScaleOut, this.transform.localScale.z);
            this.transform.DOScaleY(1, m_fDuration).SetEase(m_ease).SetDelay(m_fDelay);
        }
        if (m_fAlphaOut != 1)
        {
