...
public class ArtNum : MonoBehaviour
{
    ...
    public void Init(string strType, long num, bool bShowAddSub = false, bool isShowAni = false, bool bSetNativeSize = true, bool bFlash = false)
    {
        if (!bAwake)
        {
            Awake();
        ...
        if (bShowAddSub)
        {
            ...
            if (num > 0)
            {
                using (zstring.Block())
                {
                    go.GetComponent<Image>().sprite = Resources.Load<Sprite>(zstring.Format("num/{0}/+", strType));
                    //go.GetComponent<Image>().sprite = common.ListNum[zstring.Format("{0}-+", strType)];
        ...
        if (num == 0)
        {
            ...
            using (zstring.Block())
            {
                go.GetComponent<Image>().sprite = Resources.Load<Sprite>(zstring.Format("num/{0}/0", strType));
                // go.GetComponent<Image>().sprite = common.ListNum[zstring.Format("{0}-0", strType)];
            }
            if (bBestFit)
            {
                BestFitImageWith(cellX, go);
        ...
        else
        {
            while (num > 0)
            {
                ...
                using (zstring.Block())
                {
                    go.GetComponent<Image>().sprite = Resources.Load<Sprite>(zstring.Format("num/{0}/{1}", strType, num % 10));
                    // go.GetComponent<Image>().sprite = common.ListNum[zstring.Format("{0}-{1}", strType, num % 10)];
                }
    ...
    private void BestFitImageWith(float cellX, GameObject go)
    {
        ...
        {
            float m = go.GetComponent<RectTransform>().sizeDelta.x;
            float y = go.GetComponent<RectTransform>().sizeDelta.y;

            float rate = m / y;// ((float)go.GetComponent<Image>().sprite.texture.width) / ((float)go.GetComponent<Image>().sprite.texture.height);
