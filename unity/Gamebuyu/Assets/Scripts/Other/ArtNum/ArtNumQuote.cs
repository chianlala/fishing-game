...
public class ArtNumQuote : MonoBehaviour 
{ 
    ...
    public void Init(long num, bool bShowAddSub = false, bool isShowAni = false, bool bSetNativeSize = false, bool bFlash = false)
    {
        if (AllSprite==null)
        {
            Debug.LogError("ArtNumQuote错误图片："+this.name);
        }
        ...
        if (bShowAddSub)
        {
            ...
            if (num > 0)
            {
                using (zstring.Block())
                {
                    go.GetComponent<Image>().sprite = AllSprite[10];
                }                  
            }
            else
            {
