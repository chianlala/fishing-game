...
namespace Game.UI
{
    ...
    {
        public ShopItem(int vid, string vpayMoney,  string vshopName, int vshopType, long vshopCount)
        {
            id = vid;
            payMoney = vpayMoney;
            shopType = vshopType;
            shopName = vshopName;
            shopCount = vshopCount;
        }
        public int id;
    ...
    public class UIShop : MonoBehaviour
    {
        ...
        public List<ShopItem> list_ShopItem = new List<ShopItem>() {
            new ShopItem(9,"2","钻石",4,20),
            new ShopItem(10,"10","钻石",4,100),
            new ShopItem(11,"50","钻石",4,980),
            new ShopItem(12,"98","钻石",4,1980),
            new ShopItem(13,"198","钻石",4,3280),
            new ShopItem(14,"328","钻石",4,6480),
            new ShopItem(15,"648","钻石",4,6480),
            new ShopItem(16,"1000","钻石",4,10000),
        };
        //-------------------------view_diamond-------------------
        public Button[] btn_diamonds = new Button[7];
        //public Text[] btn_diamondsTips=new Text[4];
        //-------------------------view_gold----------------------
        public Button[] btn_golds = new Button[8];
        //public Text[] btn_goldsTips=new Text[8];
        //-------------------------view_Skill----------------------
        public Button[] btn_skill = new Button[10];

        public Button[] btn_hedans = new Button[5];
        //-------------------------view_reward--------------------
        ...
        public string WanWebServerIP = "1.13.80.54:8080/ttmy_admin/api";
        private void Awake()
        {
            InitFind();
            InitClickEvet();
        ...
        void InitFind()
        {
            Mask = this.transform.Find("mask2");
            ...
            btn_AliPay = this.transform.Find("view_PayType/Background/Content/Button_zfb").GetComponent<Button>();
            btn_AliPaySao = this.transform.Find("view_PayType/Background/Content/Button_zfbs").GetComponent<Button>();
            btn_WeiChat = this.transform.Find("view_PayType/Background/Content/Button_wx").GetComponent<Button>();
            btn_WeiChatSao = this.transform.Find("view_PayType/Background/Content/Button_wxshao").GetComponent<Button>();
        ...
        void Zhifu(int n) {
            if (CanbuyMoney <= 0)
            {
                MessageBox.Show("今日充值已达上限");
                return;
            }
            payState();
            view_PayType.gameObject.SetActive(true);
            btn_WeiChatSao.onClick.RemoveAllListeners();
            btn_WeiChatSao.onClick.AddListener(() =>
            {
                GetOrder(n, PlayerData.PlayerId, (int)common.wxS, true);
            });
            btn_WeiChat.onClick.RemoveAllListeners();
            btn_WeiChat.onClick.AddListener(() =>
            {
#if UNITY_STANDALONE_WIN
                MessageBox.Show("电脑端暂不支持", null, () => { });
#else
                GetOrder(n, PlayerData.PlayerId, (int)common.wxH5, false);
#endif
            });
            btn_AliPay.onClick.RemoveAllListeners();
            btn_AliPay.onClick.AddListener(() =>
            {
#if UNITY_STANDALONE_WIN
                MessageBox.Show("电脑端暂不支持", null, () => { });
#else
                Debug.Log("common.zfbH5" + common.zfbH5);
                GetOrder(n, PlayerData.PlayerId, (int)common.zfbH5, false);
#endif
            });
            btn_AliPaySao.onClick.RemoveAllListeners();
            btn_AliPaySao.onClick.AddListener(() =>
            {
                GetOrder(n, PlayerData.PlayerId, (int)common.zfbS, true);
            });
        }
        ...
        void GetOrder(int nIndex, long ProductId, int tpye, bool isNei)
        {
            ...
            string url = "http://" + WanWebServerIP + "/pay/unifiedorder";
          //  WanWebServerIP = "http://119.45.233.212:8080/ttmy_admin/api/pay/unifiedorder"
            JsonData requestData = new JsonData();
            requestData["payType"] = tpye;//支付类型 微信：1 支付宝：2 
            requestData["itemId"] = list_ShopItem[nIndex].id;
            requestData["tradeType"] = "APP";
            requestData["payMoney"] = list_ShopItem[nIndex].payMoney;
            requestData["playerId"] = PlayerData.PlayerId;
            requestData["shopName"] = "钻石*" + list_ShopItem[nIndex].shopCount;
            requestData["shopType"] = 4;
            requestData["shopCount"] = list_ShopItem[nIndex].shopCount;
            Debug.Log("send: "+requestData.ToJson());
            //print(requestData.ToJson());
            //StartCoroutine(GameHelper.SendHttpGetRequest(url+ "/" + requestData.ToJson(), (www) =>
            //回调的第一个为错误
            StartCoroutine(commonunity.SendHttpPostRequest(url, requestData.ToJson(), (error, allText) =>
            {
                ...
                {
                    Debug.Log(allText);
                    string result = allText;
                    JsonData responseData = JsonMapper.ToObject(result);
                    var success = responseData["success"];

                    if (success != null && (bool)success == true)
                    {
                        string str = responseData["data"].ToString();
                        Debug.Log(str);
                        JsonData reData2 = JsonMapper.ToObject(str);
                        string pay_code = reData2["code_url"].ToString();

#if UNITY_EDITOR
                    Application.OpenURL(pay_code);

#elif UNITY_ANDROID || UNITY_IOS
                      if (isNei)
                    {
                        UIRoot.Instance.trans_UIWeb.gameObject.SetActive(true);
                        WebViewUI.instance.Load(pay_code);
                    }
                    else
                    {
                        Application.OpenURL(pay_code);
                    ...
                    else
                    {
                        ...
                        MessageBox.Show("获取订单失败," + errMsg.ToString());
                        Mask.gameObject.SetActive(false);
                    }
                }
            }));
