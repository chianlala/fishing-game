using System.Collections;
using System.Collections.Generic;
using UnityEngine;
using DG.Tweening;
using UnityEngine.Profiling;
using com.maple.game.osee.proto.fishing;
using DG.Tweening.Core;
using DG.Tweening.Plugins.Core.PathCore;
using DG.Tweening.Plugins.Options;
using System.Linq;
using UnityEngine.UI;

using System;
using com.maple.game.osee.proto;
using GameFramework;
using CoreGame;


namespace Game.UI
{
     
    public class JxYinYangLongPath : MonoBehaviour
    {
        public Transform FishView; 
        public Animation Ani; 

        //是否加载了 用来播放背景的
        public bool bInit = false;
        float beforeTime = 0f;
        public List<string> AllAni = new List<string>() {
            "idle", "idle", "idle", "idle_2", "idle", "idle", "idle",
            "idle", "idle", "idle", "idle_2", "idle", "idle", "idle",
            "idle", "idle", "idle", "idle_2", "idle", "idle", "idle",
            "idle", "idle", "idle", "idle_2", "idle", "idle", "idle",
            "idle", "idle", "idle", "idle_2", "idle", "idle", "idle",
            "idle", "idle", "idle", "idle_2", "idle", "idle", "idle",
            "idle", "idle", "idle", "idle_2", "idle", "idle", "idle",
            "idle", "idle", "idle", "idle_2", "idle", "idle", "idle",
            "idle", "idle", "idle", "idle_2"
        };
        void Awake()
        {
            Ani = this.transform.Find("FishView/bs_5089_idle/mdl_model1_bs_5089_YX_idle").GetComponent<Animation>();
            FishView = this.transform.Find("FishView");
        }
        int targetI = -1;
        public void SetfLifeTime(float varfLifeTime)
        {
            //float tmpTime = 0;
            //for (int i = 0; i < AllAni.Count; i++)
            //{
            //    tmpTime += Ani[AllAni[i]].length;
            //}
            //Debug.Log("JxYinYangLongPath" + tmpTime);
            beforeTime = 0;
            targetI = -1;
            //跳转到对应时间的动画
            float mfLifeTime = 0;
            for (int i = 0; i < AllAni.Count; i++)
            {
                if (mfLifeTime >= varfLifeTime)
                {
                    targetI = i;

                    Ani.Play(AllAni[targetI]);
                    Ani[AllAni[targetI]].time = mfLifeTime - varfLifeTime;// + Ani[AllAni[targetI]].length;
                    if (Ani[AllAni[targetI]].time == 0)
                    {
                        Ani[AllAni[targetI]].time = 0.001f;
                    }
                    break;
                }
                mfLifeTime += Ani[AllAni[i]].length;
            }
            if (targetI == -1)
            {
                //说明时间时间动画已经播完
                this.gameObject.SetActive(false);
                Destroy(this.gameObject);
            }
            //if (bInit == false)
            //{
            //    bInit = true;
            //    //更换背景
            //    int bgID = common4.dicMoldConfig["阴阳龙"].BgID;
            //    if (bgID > 0)
            //    {
            //        GameObject varUIGoBg = common4.LoadPrefab("SceneBg/bg_" + bgID);
            //        var mm = Instantiate(varUIGoBg, common2.base_BG);
            //        mm.transform.localScale = Vector3.one;
            //        mm.transform.localPosition = Vector3.zero;
            //    }
            //}
            //else
            //{
            //}
        }
        public void SetfAniSpeed(float speed)
        {
            try
            {
                Ani[AllAni[targetI]].speed = speed;
            }
            catch
            {

            }

        }
        void ChangeTimeAni()
        {
            if (targetI==-1)
            {
                return;
            }
            if (targetI >= AllAni.Count)
            {
                //说明时间时间动画已经播完
                this.gameObject.SetActive(false);
                Destroy(this.gameObject);
                return;
            }
       
            if (Ani[AllAni[targetI]].time >= Ani[AllAni[targetI]].length ||  Ani[AllAni[targetI]].time == 0)
            {
                //加1后继续判断
                targetI++;
                if (targetI >= AllAni.Count)
                {
                    //说明时间时间动画已经播完
                    this.gameObject.SetActive(false);
                    Destroy(this.gameObject);
                    return;
                }

                Ani.Play(AllAni[targetI]);
                Ani[AllAni[targetI]].time = 0.01f;
            }
        }
        void FixedUpdate()
        {
            ChangeTimeAni();
        }
    }
}
